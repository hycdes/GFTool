// 玩点克制Map，存在即这个tag包含了玩点配合值
var lib_rst_tag = new Map

// Sirius
lib_rst_tag.set('gp_s_survive_shield', [
    ['gp_m_damage_breakout', 1], // 护盾 C 爆发伤害
])

// Mobius
lib_rst_tag.set('gp_m_other_area', [
    ['gp_s_hide_invi', 1], // 范围判定 C 隐身
    ['gp_s_block_watchsmoke', 1], // 范围判定 C 视觉遮蔽：烟雾
])


// 玩点克制值Map，记录tag*tag分数最高值
// 结构：< tag_name , [ tag_this_name , value ] >
var lib_rst_tag_value = new Map
// 
// 结构：< tdoll_id , [ [tag , level , tag_this , level_this , cpt_value ]... ] >
var lib_rst_record = new Map



// 玩点搭配值：
function calculate_restraint(ID) {
    if (ID === -1) true
    else {
        var display_type = ''
        var cpt = 0 // 契合度
