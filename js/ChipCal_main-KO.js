...
function changeRepo (typeInfo) { // 刷新仓库显示，1=添加，2=删除某个，3=清空
  ...
  switch (typeInfo) {
    ...
    case 2:
      ...
      deleteSelectHTML = ['<option value=0 selected>번호 선택</option>']
      for (var i = 0; i < chipNum; i++) {
        var ChartAdd = ''
        var colorName
    ...
    case 3:
      if (switch_clear === false) {
        switch_clear = true
        document.getElementById('alert_clear').innerHTML = ' * 다시 클릭을 하면 모든 칩셋을 목록에서 삭제합니다'
      } else {
        switch_clear = false
...
function maxAllChip () {
  ...
  if (switch_maxall === false) {
    switch_maxall = true
    document.getElementById('alert_maxall').innerHTML = ' * 다시 클릭을 하면 모든 칩셋을 최대 수치까지 강화합니다'
  } else {
    switch_maxall = false
    ...
    for (var c = 0; c < chipNum; c++) {
      if (chipRepo_data[c].levelNum < 20) {
        ...
        if (chipRepo_chart[c].chipType === '5칸 모양b' || chipRepo_chart[c].chipType === '5칸 모양d' || chipRepo_chart[c].chipType === '5칸 모양|' || chipRepo_chart[c].chipType === '5칸 모양C' || chipRepo_chart[c].chipType === '5칸 모양Z' || chipRepo_chart[c].chipType === '5칸 모양Z-' || chipRepo_chart[c].chipType === '5칸 모양V' || chipRepo_chart[c].chipType === '5칸 모양L' || chipRepo_chart[c].chipType === '5칸 모양L-') {
...
function blueAllChip () {
  ...
  if (switch_blueall === false) {
    switch_blueall = true
    document.getElementById('alert_maxall').innerHTML = ' * 다시 클릭을 하면 파란색 칩셋을 제외한 모든 칩셋을 목록에서 삭제합니다'
  } else {
    switch_blueall = false
    document.getElementById('alert_maxall').innerHTML = ''
    var ChipRepoChartId = document.getElementById('ChipRepoChart')
...
function orangeAllChip () {
  ...
  if (switch_orangeall === false) {
    ...
    document.getElementById('alert_maxall').innerHTML = ' * 다시 클릭을 하면 주황색 칩셋을 제외한 모든 칩셋을 목록에서 삭제합니다'
  } else {
    switch_orangeall = false
    document.getElementById('alert_maxall').innerHTML = ''
    var ChipRepoChartId = document.getElementById('ChipRepoChart')
...
function getSaveCode () { // 获取存储码
  ...
  SaveAlertId.innerHTML = '<span style="color:#FF0066">&nbsp&nbsp* 저장 코드가 클립보드에 복사되었습니다</span>'
}
function repo_addChart (chipData) {
  ...
  if (chipData.classNum === 56) {
    Repo_Type += '6칸 '
    if (chipData.typeNum === 1) Repo_Type += '모양1'
    else if (chipData.typeNum === 2) Repo_Type += '모양2'
    else if (chipData.typeNum === 3) Repo_Type += '모양3'
    else if (chipData.typeNum === 41) Repo_Type += '모양4a'
    else if (chipData.typeNum === 42) Repo_Type += '모양4b'
    else if (chipData.typeNum === 5) Repo_Type += '모양5'
    else if (chipData.typeNum === 6) Repo_Type += '모양6'
    else if (chipData.typeNum === 7) Repo_Type += '모양7'
    else if (chipData.typeNum === 8) Repo_Type += '모양8'
    else if (chipData.typeNum === 9) Repo_Type += '모양9'
  }
  else if (chipData.classNum === 551) {
    Repo_Type += '5칸 '
    if (chipData.typeNum === 11) Repo_Type += '모양Fa'
    else if (chipData.typeNum === 12) Repo_Type += '모양Fb'
    else if (chipData.typeNum === 21) Repo_Type += '모양Na'
    else if (chipData.typeNum === 22) Repo_Type += '모양Nb'
    else if (chipData.typeNum === 31) Repo_Type += '모양Ya'
    else if (chipData.typeNum === 32) Repo_Type += '모양Yb'
    else if (chipData.typeNum === 4) Repo_Type += '모양T'
    else if (chipData.typeNum === 5) Repo_Type += '모양W'
    else if (chipData.typeNum === 6) Repo_Type += '모양X'
    else if (chipData.typeNum === 81) Repo_Type += '모양b'
    else if (chipData.typeNum === 82) Repo_Type += '모양d'
    else if (chipData.typeNum === 9) Repo_Type += '모양|'
    else if (chipData.typeNum === 10) Repo_Type += '모양C'
    else if (chipData.typeNum === 111) Repo_Type += '모양Z'
    else if (chipData.typeNum === 112) Repo_Type += '모양Z-'
    else if (chipData.typeNum === 120) Repo_Type += '모양V'
    else if (chipData.typeNum === 131) Repo_Type += '모양L'
    else if (chipData.typeNum === 132) Repo_Type += '모양L-'
  }
...
function loadSaveCode () { // load save
  ...
  if (simpleCheck(LoadCode)) {
    ...
    chipRepo_chart = []; chipRepo_data = []; deleteSelectHTML = ['<option value=0 selected>번호 선택</option>']
    resetPage()
    ...
    LoadAlertId.innerHTML = '<span style="color:#FF0066">&nbsp&nbsp* 올바르지 않은 저장 코드입니다</span>'
  }
}
...
function getTopology () {
  ...
  if (topologySet.length > 0) {
    if (filter_switch) { // show sort
      ...
      document.getElementById('TopologySelect').innerHTML = '<option value=0 selected>가장 좋은 옵션은 다음과 같습니다</option>'
      ...
      var RTText = '<option value=0 selected>그림 1</option>'
      for (var i = 1; i < soluNum; i++) {
        RTText += '<option value=' + i + '>그림 ' + (i + 1) + '</option>'
      }
    ...
    var NoResultTextId = document.getElementById('NoResultText')
    NoResultTextId.innerHTML = '<span style="color:#FF0066">&nbsp&nbsp* 가능한 조합이 없습니다</span>'
    var TopologySelect = document.getElementById('TopologySelect')
    TopologySelect.disabled = true
    var RTText = '<option value=1 selected>그림 번호</option>'
  }
  var td2 = new Date()
  globaltime[0] += countMS(td1, td2)
  document.getElementById('timeText').innerHTML = '시각' + (globaltime[0] / 1000).toFixed(2) + 's'
}
...
function showAnalyze () {
  ...
  if (solutionSet.length > 0) {
    ...
    if (analyze_switch === 1) {
      document.getElementById('AnalyzeSwitch').innerHTML = '스탯 포인트 표시'
      ...
      document.getElementById('AnalyzeSwitch').innerHTML = '스탯 수치 표시'
...
function getSolution (topology) {
  ...
  for (var i = 0; i < topolen; i++) { // 记录芯片모양和对应数量
...
function sortSolution (sortType) {
  ...
  switch (ranking_switch) {
    case 1: // All property
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최대 유효 스탯</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
      solutionSet.sort(compare_sumpro)
      break
    case 2: // All blockNum
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최대 유효 포인트</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
      solutionSet.sort(compare_sumblo)
      break
    case 3: // Dmg
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최고 살상</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
      solutionSet.sort(compare_dmg)
      break
    case 4: // Dbk
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최고 파쇄</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
      solutionSet.sort(compare_dbk)
      break
    case 5: // Acu
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최고 정밀</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
      solutionSet.sort(compare_acu)
      break
    case 6: // Fil
      document.getElementById('SortInfo').innerHTML = 'Ranking as<span style="color:red"><b>최고 장전</b></span>' + ', 그림 ' + (topologyNum + 1) + ' have ' + solutionSet.length + ' 조합'
...
function setBest (typeInfo) {
  if (typeInfo === 1) {
    ...
    document.getElementById('best_alert').innerHTML = ' 【전체 조합】가능한 모든 조합을 제공합니다.(매우 빠름 + 필요한 것을 찾기 힘들 수 있습니다)'
  }else {
    filter_switch = true
    document.getElementById('best_num').disabled = false
    document.getElementById('best_alert').innerHTML = ' 【최적 조합】최적화 된 구성으로 조합을 출력합니다.(약간의 시간이 소요됩니다)'
  }
}
...
function getHelp () { window.open('../img/CC-tutorial-KO.png'); }
